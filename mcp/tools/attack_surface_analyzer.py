    ...
    async def analyze(self, host: str) -> AttackSurfaceResponse:
        ...
        if not self.api_key:
            return AttackSurfaceResponse(
                ...
                query_host=host,
                ip_address="",
                error="ZOOMEYE_API_KEY is not configured."
            )
...
async def analyze_attack_surface(**kwargs) -> dict:
    """
    Analyzes the internet-facing attack surface of a given host (IP/domain).
    This function is the entry point for the MCP server.
    """
    host_to_check = kwargs.get("host")
    if not host_to_check:
        return {"status": "error", "error": "No host provided."}
        
    response = await attack_surface_analyzer_tool.analyze(host=host_to_check)
